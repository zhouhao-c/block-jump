cc.Class({
    ...
    onLoad: function () {
        ...
        // 第6关之后方块移动加速
        if(this.gameLevel >= 3 ){
            this.player.getComponent('Player').moveDuration = 3;
        }
        if(this.gameLevel >= 6 ){
        ...
        if(this.gameLevel>0){
            ...
            levelPrefab.setPosition(cc.v2(0,600));
            levelPrefab.color = this.bg2.color;
            levelPrefab.getComponent(cc.Label).string ="第" + this.gameLevel + "关";

            this.suspend.color = this.bg2.color;
    ...
    toBong:function(x,y){
        ...
        for(var i = 0; i < n; i++){
            ...
            var move = cc.moveBy(1,cc.v2(vx,-vy)).easing(cc.easeCubicActionOut());
    ...
    restart:function(level){
        this.player.destroy();
        this.createPlayer(-(this.direction*this.node.width/2+this.player.width/2),this.node.height/2-level*this.node.height/3+32);
        this.player.getComponent('Player').mediaWidth = this.node.width;
        this.player.getComponent('Player').mediaHeight = this.node.height;
        if(this.gameLevel >= 6){
            this.player.getComponent('Player').moveDuration = 2.5;
    ...
    gameOver:function(){
        ...
        if(this.gameLevel !== 0){
            this.gameLevel++;
            cc.director.loadScene("game-level-"+this.gameLevel);
        }else{
            this.player.destroy();
        }
        // 通过最后一关后返回菜单
